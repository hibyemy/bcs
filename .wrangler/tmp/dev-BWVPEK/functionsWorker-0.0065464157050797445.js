...
// .wrangler/tmp/pages-LuleHe/functionsWorker-0.0065464157050797445.mjs
...
function createClient(input) {
  ...
  const result = {
    ...
    async verify(subjects3, token, options) {
      const jwks = await getJWKS();
      try {
        const result2 = await jwtVerify(token, jwks, {
          issuer
        });
        const validated = await subjects3[result2.payload.type]["~standard"].validate(result2.payload.properties);
        ...
        if (e instanceof errors_exports.JWTExpired && options?.refresh) {
          ...
          if (refreshed.err)
            return refreshed;
          const verified = await result.verify(subjects3, refreshed.tokens.access, {
...
async function onRequest(context) {
  ...
  const client = createClient({
    clientID: "bcs-frontend",
    issuer: "http://localhost:8789"
    // Local OpenAuth Server
  });
  try {
    const verified = await client.verify(subjects2, token);
    if (verified.err) {
...
// .wrangler/tmp/bundle-FfBTqd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-FfBTqd/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.0065464157050797445.js.map
